...
double SimpleGaussian::computeDoubleDerivative(std::vector<class Particle>& particles) {
    ...
    for(int i=0; i<m_system->getNumberOfParticles(); i++){
        ...
        for(int d = 0; d < m_system->getNumberOfDimensions() - 1; d++){
         ...
         r_i_square += particles.at(i).getPosition()[d]*
                       particles.at(i).getPosition()[d]*m_parameters[2]/(m_parameters[0]*m_parameters[0]);
       ...
       for(int j=0; j < i; j++) {
           ...
           int d = m_system->getNumberOfDimensions() - 1;
               r_ir_j += particles.at(i).getPosition()[d]*
                         particles.at(j).getPosition()[d]*
                         m_parameters[2]/(m_parameters[0]*m_parameters[0]);
...
//        potentialEnergy = r2*0.5;
        ...
        double interactionLaplacian = 0;
            ...
            for (int k=0; k < m_system->getNumberOfDimensions(); k++)
                r_i_square += m_system->getParticles()[i].getPosition()[k]*
            ...
            for (int j=0; j < i; j++) {
                ...
                sum4 -= 4 * alpha * interactionRange * r_i_square * r_iDotr_j /
            ...
            sum2 = sum1*sum1;
            sum1 *= (m_system->getNumberOfDimensions()-1)/sqrt(r_i_square);
    ...
    for (int d = 0; d < m_system->getNumberOfDimensions(); d++){
        one += (m_parameters_squared[d]*particles.at(singParticle).getPosition()[d]*particles.at(singParticle).getPosition()[d])
                -2*(m_parameters[d]);
...
//            //one -= 2*(m_system->getNumberOfDimensions())*m_parameters[0];// 2*m_parameters[2];
...
//    }
  ...
  ;
   ...
   int i=singParticle;
    ...
    return one-4*second+third-fourth;
*/
//    int k = singParticle;
//    double R_kj, R_ki;
//        for(int j=0; j<m_system->getNumberOfParticles(); j++){
...
//        }
  ...
  //cout << one+second+third*4+fourth << endl;

    //return one+second+third*4+fourth;
}
